...
email = 'toolprovjpno1vodichvutru'+ datetime.now().strftime('%y%m%d%H%M%S')+ '@gmail.com'
...
url_js_gmo = url_root + '/public/common/js/multipayment_gmo_token_test.js'

cookie_step1 = {
    'csrf_cookie_name':csrf_key
}
x=requests.get(url_js_gmo,cookies=cookie_step1,headers=headers,verify=False)
...
#     print(link)
inputs = soup_server.find_all('input')
if server_inputs:
...
inputs1 = soup_server.find_all('input')
...
if shop:
    ...
    con_shop_values = shop_values[0]
# print(con_agree)
print(con_shop_values)
